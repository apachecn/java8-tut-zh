...
SelectionDemo 示例绘制一个矩形，说明用户当前的拖动。要绘制矩形，应用程序必须为三种鼠标事件实现事件处理器：鼠标按下，鼠标拖动和鼠标释放。要获知所有这些事件，处理器必须同时实现`MouseListener`和`MouseMotionListener`接口，并注册为鼠标侦听器和鼠标移动侦听器。为了避免必须定义空方法，处理器不直接实现任何一个侦听器接口。相反，它扩展了`MouseInputAdapter`，如下面的代码片段所示。
