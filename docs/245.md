...
仅当输入字符串的第一个字符*而不是*包含字符类定义的任何字符时，匹配才会成功。
...
您还可以使用*联合*创建由两个或多个单独的字符类组成的单个字符类。要创建联合，只需将一个类嵌套在另一个类中，例如`[0-4[6-8]]`。此特定联合创建一个与数字 0,1,2,3,4,6,7 和 8 匹配的单个字符类。
...
最后，您可以使用*减法*来否定一个或多个嵌套字符类，例如`[0-9&&[^345]]`。此示例创建一个匹配 0 到 9 之间的所有字符类，*除了*数字 3,4 和 5。
