...
另一方面，您的程序可能会区分不同类型的控件 - 甚至是同一类的控件 - 并为每个控件使用不同的用户界面元素。这将要求您测试`Control's getType`方法返回的 _实例_。然后，例如，如果类型匹配`BooleanControl.Type.APPLY_REVERB`，您的程序可能会显示一个复选框;如果类型匹配`BooleanControl.Type.MUTE`，您可能会显示切换按钮。
