...
分层窗格是 Swing 容器，为定位组件提供第三维：_深度_，也称为 _Z 顺序_。将组件添加到分层窗格时，将其深度指定为整数。数字越大，组件越接近容器内的“顶部”位置。如果组件重叠，则“更近”的组件在较低深度的组件顶部绘制。相同深度处的组件之间的关系由它们在深度内的位置确定。

* * *

**Note:** 

AWT 容器有一个 API，允许您操作组件 _Z 顺序_。有关更多信息，请参阅 [AWT 焦点规范](https://docs.oracle.com/javase/8/docs/api/java/awt/doc-files/FocusSpec.html#ZOrder)。
