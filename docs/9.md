...
1.  Java SE 开发工具包 8（JDK 8）
    ...
    您现在可以[下载 Solaris OS 或 Linux 版本](http://www.oracle.com/technetwork/java/javase/downloads/index.html)。 （确保下载 **JDK** ，_而不是_ JRE。）请参阅[安装说明](https://docs.oracle.com/javase/8/docs/technotes/guides/install/install_overview.html)。
...
*   将源文件编译为.class 文件
    ...
    Java 编程语言 _编译器_（`javac`）获取源文件并将其文本转换为 Java 虚拟机可以理解的指令。此`.class`文件中包含的指令称为 _字节码_。
...
当您第一次出现提示时，您的 _当前目录_ 通常是您的 _主目录_。您可以随时在提示符下键入`cd`，然后按**返回**，将当前目录更改为主目录。
...
当你启动 Pico 时，它会显示一个新的空白 _缓冲区_。这是您键入代码的区域。
