...
当显示具有从左到右和从右到左文本混合的行时，*基本方向*是重要的。基本方向是主要书写系统的脚本顺序。例如，如果文本主要是带有一些嵌入式阿拉伯语的英语，则基本方向是从左到右。如果文本主要是阿拉伯语，其中包含一些嵌入的英语或数字，则基本方向是从右到左。
...
在可编辑文本中，*插入符*用于以图形方式表示当前插入点，即文本中将插入新字符的位置。通常，插入符号显示为两个字形之间的闪烁垂直条。插入新字符并显示在插入符号的位置。
