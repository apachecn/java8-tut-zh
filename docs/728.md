...
不可滚动的`ResultSet`对象只能使用`next`方法向前移动光标，并且只能将光标从第一行向前移动到最后一行。但是，默认的`JdbcRowSet`对象可以使用`ResultSet`接口中定义的所有光标移动方法。
...
样例`[JdbcRowSetSample](gettingstarted.html)`执行以下操作：
