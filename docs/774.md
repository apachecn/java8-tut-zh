...
文档对象模型提供的 API 允许您创建，修改，删除和重新排列节点。在尝试创建 DOM 之前，了解 DOM 的结构是很有帮助的。这一系列示例将通过名为 `DOMEcho` 的示例程序使 DOM 内部可见，您可以在目录 `_INSTALL_DIR_/jaxp-_版本_/` 中找到安装 JAXP API 后的 `samples/dom` 。
