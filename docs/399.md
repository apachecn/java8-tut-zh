...
传递给`JTextField`构造器的整数参数，示例中的`20`表示字段中的列数。此数字与字段当前字体提供的度量一起使用，以计算字段的首选宽度。它不限制用户可以输入的字符数。为此，您可以使用[格式的文本字段](formattedtextfield.html)或文档侦听器，如[文本组件功能](generaltext.html)中所述。
