...
`enum`构造器`Colors(String name, int ordinal)`是默认构造器，它是隐式声明的。但是，此构造器的形式参数（`name`和`ordinal`）是*而不是*隐式声明的。由于这些形式参数既未明确声明也未隐式声明，因此它们是合成的。 （`enum`构造的默认构造器的形式参数不是隐式声明的，因为不同的编译器不需要在这个构造器的形式上达成一致;另一个 Java 编译器可能为它指定不同的形式参数。当编译器编译使用[COD5 的表达式]时]常量，它们仅依赖于`enum`构造的公共静态字段，这些字段是隐式声明的，而不是它们的构造器或这些常量的初始化方式。）
