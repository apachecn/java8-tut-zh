...
菜单的独特之处在于，按照惯例，菜单不会与 UI 中的其他组件放在一起。相反，菜单通常出现在*菜单栏*或*弹出菜单*中。菜单栏包含一个或多个菜单，并具有通常的平台相关位置 - 通常位于窗口顶部。弹出菜单是一个不可见的菜单，直到用户在启用弹出窗口的组件上进行特定于平台的鼠标操作（例如按下鼠标右键）。然后弹出菜单出现在光标下。
...
菜单支持两种键盘选择：助记符和加速器。*助记符*提供了一种使用键盘导航菜单层次结构的方法，增加了程序的可访问性。另一方面，*加速器*提供键盘快捷键到*旁路*导航菜单层次结构。助记符适用于所有用户;加速器适用于高级用户。
