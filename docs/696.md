...
这些方法直接对应于前两个。每个返回所有支持的文件类型的数组 - 在第一种方法的情况下通常支持的所有类型，以及在第二种方法的情况下支持特定音频流的所有类型。第一种方法的典型实现可能只返回文件编写器的构造函数初始化的数组。第二种方法的实现可能会测试流的`AudioFormat`对象，以查看它是否是所请求的文件类型支持的数据格式。
