...
从`ServiceLoader`类的角度来看，所有服务都有一个类型，通常是单个接口或抽象类。供应器本身包含一个或多个具体类，这些类使用特定于其目的的实现来扩展服务类型。 `ServiceLoader`类要求单个公开的供应器类型具有默认构造函数，该构造函数不需要参数。这使`ServiceLoader`类可以轻松实例化它找到的服务提供者。
...
`ServiceLoader` API 很有用，但它有局限性。例如，无法从`ServiceLoader`类派生类，因此您无法修改其行为。您可以使用自定义`ClassLoader`子类来更改类的查找方式，但不能扩展`ServiceLoader`本身。此外，当运行时新的供应器可用时，当前的`ServiceLoader`类无法告诉您的应用程序。此外，您无法将更改侦听器添加到加载程序以查明是否将新供应器放入特定于应用程序的扩展目录中。
